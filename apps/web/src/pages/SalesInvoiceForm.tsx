...
import { salesInvoicesApi, type SalesInvoice } from '@lib/api/endpoints/sales-invoices';
...
export default function SalesInvoiceFormPage() {
  ...
  async function handleSave() {
    if (!customerId) {
      message.error(t('validation.required', { field: t('salesInvoices.customer') }));
      return;
    }
    setSaving(true);
    try {
  ...
  async function handleSubmit() {
    ...
    setSaving(true);
    try {
      await salesInvoicesApi.submit(id);
      message.success(t('common.updated'));
      navigate('/sales-invoices');
    } catch {
      message.error(t('errors.saveError'));
  ...
  const lineColumns = [
    ...
    {
      title: t('salesInvoices.quantity'),
      dataIndex: 'quantity',
      key: 'quantity',
      width: 100,
      render: (_: unknown, __: unknown, index: number) => (
    ...
    {
      title: t('salesInvoices.discountPct'),
    ...
    {
      title: t('salesInvoices.lineTotal'),
      dataIndex: 'amount',
      key: 'amount',
      width: 120,
  ...
  return (
    <div className="erpnext-form">
      <ErpFormHeader
        title={
          isEdit ? invoice?.invoiceNumber || t('salesInvoices.edit') : t('salesInvoices.create')
        }
        status={isEdit ? status : undefined}
        statusLabel={isEdit ? t(`salesInvoices.status_${status}`) : undefined}
        isDirty={false}
              ...
              if (tab === 'activity') {
                return (
                  <div style={{ fontSize: 12, color: '#8d99a6' }}>
                    <p>
                      {t('common.status')}: {t(`salesInvoices.status_${status}`)}
                    </p>
                    {invoice?.createdAt && (
                      <p>
                        {t('common.createdAt')}:{' '}
      ...
      >
        ...
        >
          <ErpFieldGrid>
            <ErpField label={t('salesInvoices.customer')} required>
              <Select
                showSearch
                placeholder={t('salesInvoices.selectCustomer')}
            ...
            <ErpField label={t('salesInvoices.invoiceDate')}>
              <DatePicker
                style={{ width: '100%' }}
                value={invoiceDate}
            ...
            <ErpField label={t('salesInvoices.dueDate')}>
              <DatePicker
                style={{ width: '100%' }}
                value={dueDate}
            ...
            <ErpField label={t('salesInvoices.notes')} fullWidth>
